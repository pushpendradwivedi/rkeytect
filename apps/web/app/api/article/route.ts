...
export async function POST(request: NextRequest) {
  // Security invariant: this route accepts a public AWS URL only. It never accepts an AI API key.
  ...
  if (!url) return NextResponse.json({ error: "Only HTTPS AWS Blog URLs are supported." }, { status: 400 });

  const response = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; rkeytect/0.1; +https://github.com/pushpendradwivedi/rkeytect)",
  ...
  if (!response.ok) return NextResponse.json({ error: `AWS Blog returned ${response.status}.` }, { status: 502 });
